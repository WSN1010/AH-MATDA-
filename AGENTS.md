...
6. **구현 가이드의 마일스톤 순서를 따른다**: 백엔드 B1(로컬 Copilot SDK 실증)은 B4의 차단 게이트다. Azure 호스팅 검증(B7)은 B4~B6를 차단하지 않으며, 수행 전에는 통과로 서술하지 않는다.
7. **로컬 백엔드는 컨테이너 런타임에 의존하지 않는다**: AppHost가 API, Worker, Azure Storage 개발 리소스를 .NET Aspire로 오케스트레이션한다. Docker 및 `RunAsEmulator()`를 사용하지 않는다.
